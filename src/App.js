...
import { 
  Wallet, User, Plus, Trash2, LogOut, 
  Utensils, Plane, Home, ShoppingBag, Bus, Sparkles, 
...
const USERS = [
  ...
  { name: '樂樂', fullName: '袁紹軒', icon: Baby, color: 'bg-amber-500', isChild: true }
];

const EXCHANGE_RATE_TWD_TO_KRW = 42.5;

const CATEGORIES = [
  { name: '飲食', icon: Utensils }, { name: '交通', icon: Bus },
  { name: '住宿', icon: Home }, { name: '購物', icon: ShoppingBag },
  { name: '機票', icon: Plane }
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubPacking = onSnapshot(
      ...
      (err) => console.error("讀取行李清單失敗", err)
    );
  ...
  const togglePackingItem = async (item, field) => {
    await setDoc(doc(db, 'artifacts', PROJECT_ID, 'packingItems', item.id.toString()), { ...item, [field]: !item[field] });
  };
  const deletePackingItem = async (id) => {
    await deleteDoc(doc(db, 'artifacts', PROJECT_ID, 'packingItems', id.toString()));
  };
    ...
    return (
      <div className="min-h-screen bg-stone-100 flex flex-col items-center justify-center p-4 font-sans text-stone-800">
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-xl w-full max-w-sm text-center border border-stone-200/50 relative overflow-hidden">
          ...
          <h1 className="text-2xl font-black text-stone-900 mb-2 tracking-tight relative z-10">Busan Trip.</h1>
          <p className="text-stone-400 mb-8 text-sm font-medium relative z-10">家族旅行財務與準備嚮導</p>
            ...
            {USERS.map((user) => {
              ...
              return (
                <button 
                  ...
                  className={`py-4 px-2 bg-stone-50 hover:bg-stone-100 border border-stone-200/60 rounded-2xl transition-all flex flex-col items-center gap-3 active:scale-95 group ${user.isChild ? 'col-span-2 mx-10 border-amber-200 bg-amber-50' : ''}`}
                >
                  <div className={`${user.color} text-white w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg shadow-sm group-hover:scale-110 transition-transform`}>
                    {user.initial ? user.initial : <IconComp className="w-6 h-6" strokeWidth={2} />}
                  </div>
                  <div className="flex flex-col items-center">
                    <span className="font-bold text-stone-700 text-sm">{user.name}</span>
                    {user.fullName && <span className="text-[10px] text-stone-400 font-medium mt-0.5">{user.fullName}</span>}
                  </div>
                </button>
              )
            })}
    ...
    <div className="min-h-screen bg-stone-100 max-w-md mx-auto relative font-sans overflow-x-hidden text-stone-800 antialiased shadow-2xl">
      ...
      <header className="bg-stone-900 text-white pt-12 pb-5 px-6 rounded-b-[2rem] shadow-lg z-10 flex-shrink-0 relative">
        <div className="flex justify-between items-center mb-1 relative z-10">
          ...
          <button onClick={handleLogout} className="p-2.5 bg-stone-800 rounded-full hover:bg-stone-700 transition-colors">
      ...
      <nav className="fixed bottom-0 w-full max-w-md bg-white/95 backdrop-blur-md border-t border-stone-200 flex justify-around p-2 pb-[calc(env(safe-area-inset-bottom)+1rem)] z-40">
        <button onClick={() => setActiveTab('expenses')} className={`flex flex-col items-center p-2 w-1/2 rounded-2xl transition-all duration-300 ${activeTab === 'expenses' ? 'text-stone-900 bg-stone-100' : 'text-stone-400 hover:bg-stone-50'}`}>
          <Wallet className="w-6 h-6 mb-1" strokeWidth={activeTab === 'expenses' ? 2.5 : 1.5} />
          <span className="text-[10px] font-bold tracking-wide">記帳結算</span>
        </button>
        <button onClick={() => setActiveTab('tools')} className={`flex flex-col items-center p-2 w-1/2 rounded-2xl transition-all duration-300 ${activeTab === 'tools' ? 'text-stone-900 bg-stone-100' : 'text-stone-400 hover:bg-stone-50'}`}>
          <Package className="w-6 h-6 mb-1" strokeWidth={activeTab === 'tools' ? 2.5 : 1.5} />
          <span className="text-[10px] font-bold tracking-wide">旅行百寶箱</span>
        </button>
      </nav>

      {showCalculator && <QuickCalculatorModal onClose={() => setShowCalculator(false)} />}
    </div>
  );
}
...
function QuickCalculatorModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/40 z-[9999] flex items-end justify-center sm:items-center p-4 pb-20 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl animate-slide-up relative" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-5">
          <h3 className="font-black text-stone-800 flex items-center gap-2"><Calculator className="w-5 h-5 text-stone-500" /> 快速匯率換算</h3>
          <button onClick={onClose} className="text-stone-400 hover:bg-stone-100 p-1.5 rounded-full"><span className="font-bold text-xs px-1">✕</span></button>
        </div>
        <div className="space-y-4">
          <div className="bg-stone-50 p-4 rounded-2xl border border-stone-200">
            <label className="block text-[10px] font-bold text-stone-400 mb-1 uppercase tracking-widest">輸入韓元標價 (KRW)</label>
            <div className="flex items-center text-3xl font-black text-stone-800">
              <span className="mr-2 text-stone-400">₩</span>
              <input type="number" value={krw} onChange={e => setKrw(e.target.value)} placeholder="0" className="w-full bg-transparent outline-none" autoFocus />
            </div>
          </div>
          <div className="flex justify-center"><ArrowDown className="w-5 h-5 text-stone-300" /></div>
          <div className="bg-stone-900 p-4 rounded-2xl border border-stone-800 text-white">
            <label className="block text-[10px] font-bold text-stone-400 mb-1 uppercase tracking-widest">大約台幣 (TWD)</label>
            <div className="flex items-center text-4xl font-black">
              <span className="mr-2 text-stone-400">$</span>
              <span>{krw ? Math.round(parseFloat(krw) / EXCHANGE_RATE_TWD_TO_KRW).toLocaleString() : '0'}</span>
            </div>
          </div>
          <p className="text-center text-[10px] text-stone-400 font-medium pt-2">此換算不留紀錄。參考匯率 1 : {EXCHANGE_RATE_TWD_TO_KRW}</p>
        </div>
      </div>
    </div>
  );
...
function ExpenseView({ expenses, currentUser, onAddExpense, onDeleteExpense, onToggleSettle }) {
  ...
  const handleKRWChange = (e) => { const val = e.target.value; setAmountKRW(val); if (val) setAmountTWD(Math.round(parseFloat(val) / EXCHANGE_RATE_TWD_TO_KRW).toString()); else setAmountTWD(''); };
  const handleExtraTWDChange = (e) => { const val = e.target.value; setExtraAmountTWD(val); if (val) setExtraAmountKRW(Math.round(parseFloat(val) * EXCHANGE_RATE_TWD_TO_KRW).toString()); else setExtraAmountKRW(''); };
  const handleExtraKRWChange = (e) => { const val = e.target.value; setExtraAmountKRW(val); if (val) setExtraAmountTWD(Math.round(parseFloat(val) / EXCHANGE_RATE_TWD_TO_KRW).toString()); else setExtraAmountTWD(''); };
    ...
    <div className="p-4 pb-10">
        ...
        <div className="flex justify-between items-start mb-6 relative z-10">
          <div>
            <p className="text-stone-400 text-[10px] font-bold uppercase tracking-widest mb-1 flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>待結算總金額
            </p>
        ...
        <div className="bg-stone-800/50 rounded-2xl p-4 border border-stone-700/50 relative z-10">
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.entries(personalCosts).map(([name, cost]) => (
              <div key={name} className="flex justify-between items-center bg-stone-800/80 px-3 py-2 rounded-lg">
                <span className="text-xs font-bold text-stone-300">{name}</span>
                <span className="text-sm font-black text-white">${Math.round(cost).toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-3 mb-8">
        {pendingExpenses.length === 0 ? (
          <div className="text-center py-10 bg-white rounded-[1.5rem] border border-stone-200 border-dashed">
            <CheckCircle className="w-10 h-10 mx-auto mb-2 text-emerald-300" strokeWidth={1.5} />
            <p className="text-sm font-bold text-stone-400">目前無待結算帳款！</p>
          </div>
        ) : (
      ...
      {settledExpenses.length > 0 && (
        <div className="mt-8 border-t border-stone-200 pt-6">
          <button onClick={() => setShowSettledHistory(!showSettledHistory)} className="w-full flex items-center justify-between px-2 text-stone-500 hover:text-stone-800 transition-colors">
            <h3 className="font-bold text-sm tracking-tight flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-500" /> 已結清歷史紀錄 ({settledExpenses.length})</h3>
        ...
        <div className="fixed inset-0 bg-stone-900/60 z-[9999] flex items-end justify-center sm:items-center p-4 pb-0 backdrop-blur-sm">
          <div className="bg-white rounded-t-[2rem] sm:rounded-[2rem] p-6 w-full max-w-md max-h-[95vh] overflow-y-auto animate-slide-up shadow-2xl">
            ...
            <div className="space-y-6 pb-6">
              <div>
                <label className="block text-[10px] font-bold text-stone-400 mb-2 uppercase tracking-widest">誰先墊錢的？(Payer)</label>
                <div className="flex gap-2">
                  {USERS.filter(u => !u.isChild).map(u => (
                    <button key={u.name} onClick={() => setPayer(u.name)} className={`flex-1 py-2.5 rounded-xl font-bold text-xs border transition-all ${payer === u.name ? 'bg-stone-900 text-white border-stone-900 shadow-md' : 'bg-white border-stone-200 text-stone-500 hover:bg-stone-50'}`}>{u.initial}</button>
                  ))}
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold text-stone-400 mb-2 uppercase tracking-widest">花費名稱</label>
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="例如：海雲台海鮮餐廳..." className="w-full px-4 py-3.5 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-stone-900 font-bold text-stone-800 text-sm" />
              </div>

              <div className="grid grid-cols-2 gap-3 bg-stone-50 p-4 rounded-2xl border border-stone-200">
                <div>
                  <label className="block text-[10px] font-bold text-stone-400 mb-2 uppercase tracking-widest">總金額 韓元 (KRW)</label>
                  <div className="relative">
                    <span className="absolute left-3 top-3.5 font-bold text-stone-400">₩</span>
                    <input type="number" value={amountKRW} onChange={handleKRWChange} placeholder="0" className="w-full pl-8 pr-3 py-3 bg-white border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-stone-900 font-black text-stone-800 text-sm" />
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-stone-400 mb-2 uppercase tracking-widest">總金額 台幣 (TWD)</label>
                  <div className="relative">
                    <span className="absolute left-3 top-3.5 font-bold text-stone-400">$</span>
                    <input type="number" value={amountTWD} onChange={handleTWDChange} placeholder="0" className="w-full pl-8 pr-3 py-3 bg-white border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-stone-900 font-black text-stone-800 text-sm" />
                  </div>
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold text-stone-400 mb-2 uppercase tracking-widest">剩餘金額由誰負擔？(Main Split)</label>
                <div className="grid grid-cols-2 gap-2">
                  {SPLIT_OPTIONS.map(opt => {
                    const OptIcon = opt.icon; const isActive = splitType === opt.id;
                    return (
                      <button key={opt.id} onClick={() => setSplitType(opt.id)} className={`flex items-start gap-2 p-3.5 border rounded-xl transition-all text-left ${isActive ? 'bg-blue-50 border-blue-200 ring-1 ring-blue-500' : 'bg-white border-stone-200 hover:bg-stone-50'}`}>
                        <OptIcon className={`w-4 h-4 flex-shrink-0 mt-0.5 ${isActive ? 'text-blue-600' : 'text-stone-400'}`} strokeWidth={2} />
                        <div>
                          <p className={`font-bold text-xs leading-tight ${isActive ? 'text-blue-900' : 'text-stone-600'}`}>{opt.label}</p>
                          <p className={`text-[9px] mt-1 font-bold ${isActive ? 'text-blue-500' : 'text-stone-400'}`}>{opt.desc}</p>
                        </div>
                      </button>
                    );
                  })}
              ...
              <div className="pt-2 border-t border-stone-100">
                <button onClick={() => setHasExtra(!hasExtra)} className="flex items-center gap-2 text-xs font-bold text-orange-600 bg-orange-50 hover:bg-orange-100 px-4 py-3 rounded-xl transition-colors w-full border border-orange-100">
                  {hasExtra ? <ChevronUp className="w-4 h-4"/> : <Plus className="w-4 h-4"/>}
                  + 包含特定家族的專屬花費嗎？(例如兒童餐)
                </button>
                ...
                {hasExtra && (
                  <div className="mt-3 p-4 bg-orange-50/50 rounded-2xl border border-orange-200 space-y-4 animate-slide-up">
                    <div>
                      <label className="block text-[10px] font-bold text-orange-700 mb-2 uppercase tracking-widest">這筆專屬費用是誰的？</label>
                      <div className="flex gap-2">
                        <button onClick={() => setExtraTarget('邱袁共同')} className={`flex-1 py-2.5 rounded-xl font-bold text-xs border transition-all ${extraTarget === '邱袁共同' ? 'bg-orange-500 text-white border-orange-600 shadow-sm' : 'bg-white border-orange-200 text-orange-600 hover:bg-orange-100'}`}>邱袁家 (含樂樂)</button>
                        <button onClick={() => setExtraTarget('黃馬共同')} className={`flex-1 py-2.5 rounded-xl font-bold text-xs border transition-all ${extraTarget === '黃馬共同' ? 'bg-blue-500 text-white border-blue-600 shadow-sm' : 'bg-white border-blue-200 text-blue-600 hover:bg-blue-50'}`}>黃馬家</button>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold text-orange-700 mb-1.5 uppercase tracking-widest">專屬金額 (KRW)</label>
                        <div className="relative">
                          <span className="absolute left-3 top-3 font-bold text-orange-400">₩</span>
                          <input type="number" value={extraAmountKRW} onChange={handleExtraKRWChange} placeholder="0" className="w-full pl-8 pr-3 py-2.5 bg-white border border-orange-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 font-black text-orange-900 text-sm" />
                        </div>
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-orange-700 mb-1.5 uppercase tracking-widest">專屬金額 (TWD)</label>
                        <div className="relative">
                          <span className="absolute left-3 top-3 font-bold text-orange-400">$</span>
                          <input type="number" value={extraAmountTWD} onChange={handleExtraTWDChange} placeholder="0" className="w-full pl-8 pr-3 py-2.5 bg-white border border-orange-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 font-black text-orange-900 text-sm" />
                        </div>
                      </div>
                    </div>
                    {isExtraInvalid && <p className="text-xs text-red-500 font-bold bg-red-50 p-2 rounded-lg flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5"/> 專屬金額不能大於或等於總金額喔！</p>}
                  </div>
                )}
              </div>

              <div>
                <label className="block text-[10px] font-bold text-stone-400 mb-2 uppercase tracking-widest">分類標籤</label>
                <div className="flex gap-2 overflow-x-auto pb-1 hide-scrollbar">
                  {CATEGORIES.map(c => {
                    const Icon = c.icon;
                    return (
                      <button key={c.name} onClick={() => setCategory(c.name)} className={`flex-shrink-0 px-4 py-3 rounded-xl flex items-center gap-1.5 transition-all font-bold text-xs border ${category === c.name ? 'bg-stone-900 text-white border-stone-900' : 'bg-white border-stone-200 text-stone-600 hover:bg-stone-50'}`}><Icon className="w-3.5 h-3.5" strokeWidth={2} /> {c.name}</button>
                    );
                  })}
                </div>
              </div>
      ...
      {showSettlement && (
        <div className="fixed inset-0 bg-stone-900/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-md" onClick={() => setShowSettlement(false)}>
                 ...
                 <div className="space-y-3">
                   {calculateSettlement().map((t, i) => (
                     <div key={i} className="bg-white p-4 rounded-2xl border border-stone-200 flex items-center justify-between shadow-sm">
                       ...
                       <span className="font-black text-lg text-red-500">${t.amount.toLocaleString()}</span>
                     </div>
                   ))}
                 </div>
               )}
...
function PackingListView({ packingItems, onAddPackingItem, onTogglePackingItem, onDeletePackingItem }) {
  ...
  const availableRecommended = recommendedList.filter(itemStr => !packingItems.some(p => p.name === itemStr));

  const toggleHM = (id) => {
    const item = packingItems.find(i => i.id === id);
    if(item) onTogglePackingItem(item, 'checkedHM');
  };
  const toggleCY = (id) => {
    const item = packingItems.find(i => i.id === id);
    if(item) onTogglePackingItem(item, 'checkedCY');
  };

  const handleAddCustom = () => {
    if (!newItemName.trim()) return;
    onAddPackingItem({ name: newItemName.trim(), checkedHM: false, checkedCY: false });
    setNewItemName('');
  };

  const handleAddRecommended = (itemStr) => {
    onAddPackingItem({ name: itemStr, checkedHM: false, checkedCY: false });
    setRecommendedList(prev => prev.filter(i => i !== itemStr));
  };
    ...
    <div className="p-4 pb-10">
      <div className="bg-gradient-to-br from-stone-800 to-stone-900 rounded-[2rem] p-6 text-white mb-6 shadow-lg relative overflow-hidden">
         ...
         <p className="text-xs text-stone-400 font-medium leading-relaxed">
           輸入各自需要帶的東西。<br/>右側可分別標記「黃馬家」與「邱袁家」是否準備完成。
         </p>
      </div>

      <div className="mb-6 flex gap-2">
        <input type="text" value={newItemName} onChange={e => setNewItemName(e.target.value)} placeholder="輸入你們想到的必備物品..." className="flex-1 px-4 py-3.5 bg-white border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-stone-900 font-bold text-stone-800 text-sm shadow-sm" />
        <button onClick={handleAddCustom} disabled={!newItemName.trim()} className="bg-stone-900 text-white px-5 rounded-xl font-bold flex items-center justify-center disabled:opacity-50 transition-all active:scale-95 shadow-sm hover:bg-stone-800">新增</button>
      </div>
      ...
      <div className="space-y-3 mb-8">
        {packingItems.length === 0 ? (
          ...
          packingItems.map(item => (
            <div key={item.id} className="bg-white p-3.5 pr-2 rounded-[1.5rem] shadow-sm border border-stone-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 group">
              <div className="flex items-center gap-2 flex-1 pl-1">
                <span className={`font-bold text-sm ${item.checkedHM && item.checkedCY ? 'text-stone-400 line-through' : 'text-stone-800'}`}>{item.name}</span>
              </div>
              <div className="flex items-center gap-2 justify-end">
                <button onClick={() => toggleHM(item.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-bold text-[11px] transition-all ${item.checkedHM ? 'bg-blue-600 border-blue-600 text-white shadow-sm' : 'bg-stone-50 border-stone-200 text-stone-400 hover:bg-stone-100'}`}>
                  {item.checkedHM ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Circle className="w-3.5 h-3.5" />} 黃馬
                </button>
                <button onClick={() => toggleCY(item.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-bold text-[11px] transition-all ${item.checkedCY ? 'bg-amber-500 border-amber-500 text-white shadow-sm' : 'bg-stone-50 border-stone-200 text-stone-400 hover:bg-stone-100'}`}>
                  {item.checkedCY ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Circle className="w-3.5 h-3.5" />} 邱袁
                </button>
                <button onClick={() => onDeletePackingItem(item.id)} className="p-1.5 text-stone-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors ml-1"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))
        )}
      ...
      {availableRecommended.length > 0 && (
        <div>
          <h3 className="text-[11px] font-bold text-stone-400 uppercase tracking-widest mb-3 flex items-center gap-1.5 px-1"><Sparkles className="w-3.5 h-3.5" /> 釜山推薦必備清單</h3>
          <div className="flex flex-wrap gap-2">
            {availableRecommended.map((itemStr, idx) => (
              <button key={idx} onClick={() => handleAddRecommended(itemStr)} className="bg-white border border-stone-200 text-stone-600 px-3.5 py-2 rounded-xl text-xs font-bold hover:bg-stone-50 hover:border-stone-300 transition-colors flex items-center gap-1.5 shadow-sm active:scale-95">
